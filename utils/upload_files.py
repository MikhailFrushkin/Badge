    ...
    async with session.get(upload_url, headers=headers, params=params) as response:
        ...
        if "href" in upload_data:
            async with session.put(
                upload_data["href"], data=open(file_path, "rb")
            ) as upload_response:
                if upload_response.status == 201:
                    ...
                    print(
                        f"Произошла ошибка при загрузке файла {destination_path}: {upload_response.text}"
                    )
        else:
            print(
                f"Произошла ошибка при получении URL для загрузки файла {destination_path}: {upload_data}"
            )
