...
def combine_images_to_pdf(
    input_files, output_pdf, size=None, progress=None, self=None, A3_flag=False
):
    ...
    if A3_flag:
        ...
        x_positions = [
            ...
            x_offset + 2 * (img_width + 10),
            x_offset + 3 * (img_width + 10),
            x_offset + 4 * (img_width + 10),
            x_offset + 5 * (img_width + 10),
        ]
        y_positions = [
            A3[0] - y_offset,
            A3[0] - y_offset - img_height - 15,
            A3[0] - y_offset - 2 * (img_height + 15),
        ]
        ...
        x_positions = [
            x_offset,
            x_offset + img_width + 5,
            x_offset + 2 * (img_width + 5),
        ]
        y_positions = [
            A4[1] - y_offset,
            A4[1] - y_offset - img_height - 10,
        ...
        for page in range(num_pages):
            ...
            for i, img in enumerate(input_files[start_idx:end_idx]):
                if not os.path.exists(img.skin):
                    ...
                    try:
                        logger.success(
                            f"Добавился скин {img.num_on_list}     {img.art}"
                        )
                        progress.update_progress()
                        c.drawImage(
                            img.skin,
                        ...
                        try:
                            ...
                            logger.error(
                                f"Не удалось добавить подложку для {img.art} {ex} 2й раз"
                            )
...
def create_contact_sheet(
    images=None, size=None, self=None, A3_flag=False, popsocket=False
):
        ...
        try:
            ...
            for i in range(config[f"{str(size)}"]["ICONS_PER_COL"]):
                for j in range(config[f"{str(size)}"]["ICONS_PER_ROW"]):
                    try:
                        image = Image.open(
                            img[i * config[f"{str(size)}"]["ICONS_PER_ROW"] + j][
                                0
                            ].strip()
                        )
                        image = write_images_art(
                            image,
                    ...
                    try:
                        if size == 56:
                            contact_sheet.paste(
                                image,
                                (j * image_width - 10, i * image_height + 10 * (i + 1)),
                            )
                            border_rect = [
                                j * image_width - 10,
                                i * image_height + 10 * (i + 1),
                                (j + 1) * image_width - 10,
                                (i + 1) * image_height + 10 * (i + 1),
                            ]
            ...
            image = write_images_art2(
                contact_sheet, f"{self.name_doc} Стр.{index}", x, y
            )
    ...
    try:
        ...
        for size in records:
            ...
            try:
                ...
                merge_pdfs_stickers(
                    queryset.order_by(Orders.num_on_list), f"Файлы на печать\\ШК {size}"
                )
            except Exception as ex:
                logger.error(ex)
                logger.error(
                    f"Не удалось создать файл ШК {size}, возможно не одного не найденно"
                )
        ...
        if len(queryset) > 0:
            ...
            try:
                ...
                logger.error(ex)
                logger.error(
                    f"Не удалось создать файл ШК Popsocket, возможно не одного не найденно"
                )
        ...
        for record in records:
            ...
            record.save()
            Statistic.create(
                art=record.art, nums=record.nums_in_folder, size=record.size
            )
    except Exception as ex:
        logger.error(ex)
