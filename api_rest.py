...
def get_info_publish_folder(public_url):
    ...
    if res.status_code == 200:
        ...
        for i in data:
            ...
            if (
                os.path.splitext(file_name)[0].isdigit()
                or "подл" in file_name
                or file_name.endswith(".pdf")
            ):
                result_data.append(
                    {"name": i.get("name").strip(), "file": i.get("file")}
                )
                    ...
                    if item["the_same"]:
                        try:
                            if os.path.exists(os.path.join(folder, "1.png")):
                                ...
                                logger.error(
                                    f"Нет файла для копирования артикул: {item}"
                                )
                                continue
                        except Exception as ex:
                            logger.error(ex)
                    ...
                    try:
                        for file in os.listdir(folder):
                            if file.endswith(".pdf"):
                                try:
                                    shutil.copy2(
                                        os.path.join(folder, file), sticker_path_all
                                    )
                                except Exception as ex:
                                    logger.error(ex)
                                else:
